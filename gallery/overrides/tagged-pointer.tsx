...
export type TaggedPointerTrigger = "always" | "hover" | "press";

export interface TaggedPointerClassNames {
...
export interface TaggedPointerProps extends Omit<
...
> {
  ...
  classNames?: TaggedPointerClassNames;
...
const TaggedPointer = forwardRef<HTMLDivElement, TaggedPointerProps>(
  function TaggedPointer(
    {
      children,
      name = "Sophie",
      arrow,
    ...
    const lastLabelRotate = useRef(0);
    ...
    const arrowRotate = useTransform<number, number>(
      ...
      ([vx, vy]) => {
        ...
        // foxbits: hold the last direction at idle. Upstream scales the delta by
        // a speed factor k, so as velocity decays the target drifts back to the
        // resting tilt before any threshold is reached — which is exactly the
        // "returns on idle" the override removes. Full direction while moving,
        // frozen cache below the threshold.
        if (speed < VELOCITY_MIN) return lastArrowRotate.current;
        const dir = (Math.atan2(vy, vx) * 180) / Math.PI;
        const target = dir - ARROW_NATURAL_DEG;
        let delta = target - tilt;
        delta = ((delta + 540) % 360) - 180;
        lastArrowRotate.current = tilt + delta;
        return lastArrowRotate.current;
      },
    );
    ...
    const labelRotate = useTransform<number, number>(
      ...
      ([vx, vy]) => {
        ...
        const speed = Math.hypot(vx, vy);
        if (speed < VELOCITY_MIN) return lastLabelRotate.current;
        const k = Math.min(
          1,
