...
void main() {
  ...
  for (int i = 0; i < ${MAX_FILAMENTS}; i++) {
    ...
    float h = fi / float(uFilamentCount) + uv.x * 0.30 + uTime * 0.04 + uHueShift;
    col += filamentColor(h) * g * env;
  }
...
export interface FilamentWeaveProps {
  colors?: string[];
  count?: number;
  speed?: number;
  amplitude?: number;
...
export default function FilamentWeave({
  ...
  const propsRef = useRef<Required<Omit<FilamentWeaveProps, 'className' | 'style'>>>({
    colors,
    count,
    speed,
    amplitude,
  ...
  useEffect(() => {
    ...
    const program = new Program(gl, {
      ...
      uniforms: {
        ...
        uFilamentCount: { value: Math.min(propsRef.current.count, MAX_FILAMENTS) },
        uSpeed: { value: speed },
    ...
    const update = (t: number) => {
      ...
      program.uniforms.uFilamentCount.value = Math.min(Math.max(Math.round(current.count), 1), MAX_FILAMENTS);
  ...
  return <div ref={ctnDom} className={`filament-weave-container ${className}`} style={style} />;
}
