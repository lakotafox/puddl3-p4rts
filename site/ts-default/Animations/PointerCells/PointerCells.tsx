...
import './PointerCells.css';

type Falloff = 'linear' | 'smooth' | 'sharp';

export interface PointerCellsProps {
  cellSize?: number;
  color?: string;
  radius?: number;
  falloff?: Falloff;
...
const PointerCells = ({
  ...
  return (
    <div ref={containerRef} className={`pointer-cells${className ? ` ${className}` : ''}`}>
      <canvas ref={canvasRef} className="pointer-cells__canvas" />
    </div>
  );
};

export default PointerCells;
