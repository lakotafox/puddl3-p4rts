...
export type ShardSwitchPattern =
  | 'random'
  | 'center'
  | 'edges'
  | 'left-to-right'
...
export type ShardSwitchTrigger = 'hover' | 'click' | 'manual';

export interface ShardSwitchProps {
  ...
  pattern?: ShardSwitchPattern;
  randomness?: number;
  easing?: string;
  trigger?: ShardSwitchTrigger;
...
const PATTERNS: Record<ShardSwitchPattern, (x: number, y: number) => number | null> = {
  random: () => null,
...
function ShardSwitch({
  ...
  useEffect(() => {
    ...
    frozenGrid.pixels.forEach((pixel, index) => {
      ...
      content.className = 'shard-switch__pixel-content';
  ...
  return (
    <div
      ref={containerRef}
      className={`shard-switch ${className}`.trim()}
    ...
    >
      ...
      {transition && (
        <div className="shard-switch__grid" aria-hidden="true">
