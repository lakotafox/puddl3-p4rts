...
import './GlowColumn.css';

interface GlowColumnProps {
  topColor?: string;
  bottomColor?: string;
  intensity?: number;
...
const GlowColumn: React.FC<GlowColumnProps> = ({
  topColor = '#5227FF',
  bottomColor = '#FF9FFC',
  intensity = 1.0,
  rotationSpeed = 0.3,
  ...
  if (!webGLSupported) {
    return (
      <div className={`glow-column-fallback ${className}`} style={{ mixBlendMode }}>
        WebGL not supported
      </div>
    );
  }

  return <div ref={containerRef} className={`glow-column-container ${className}`} style={{ mixBlendMode }} />;
};

export default GlowColumn;
