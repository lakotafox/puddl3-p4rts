...
import './SquareBurst.css';

type SquareBurstVariant = 'square' | 'circle' | 'triangle' | 'diamond';
...
type SquareBurstProps = {
  variant?: SquareBurstVariant;
  pixelSize?: number;
  color?: string;
  className?: string;
...
const SHAPE_MAP: Record<SquareBurstVariant, number> = {
  square: 0,
  circle: 1,
  triangle: 2,
  diamond: 3
...
const SquareBurst: React.FC<SquareBurstProps> = ({
  variant = 'square',
  pixelSize = 3,
  color = '#B497CF',
  className,
  ...
  return (
    <div
      ref={containerRef}
      className={`square-burst-container ${className ?? ''}`}
      style={style}
      aria-label="SquareBurst interactive background"
    />
  );
};

export default SquareBurst;
