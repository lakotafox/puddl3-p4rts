...
export interface MeltReelProps {
  items?: MorphItem[];
  startIndex?: number;
...
class MorphEngine {
  ...
  constructor(container: HTMLElement, config: EngineConfig) {
    ...
    this.canvas.className = 'melt-reel-canvas';
...
export default function MeltReel({
  items = DEFAULT_ITEMS,
  startIndex = 0,
  transition = 'melt',
  duration = 1.1,
  ...
  return (
    <div
      className={`melt-reel ${className}`.trim()}
      style={
        {
    ...
    >
      ...
      {showCaptions && hasCaptions && (
        <div className="melt-reel-caption" aria-live="polite">
          {items.map((item, i) =>
            item.caption ? (
              <span
                key={i}
                aria-hidden={i === index ? undefined : true}
                className={`melt-reel-caption-text ${i === index ? 'is-active' : ''}`}
              >
                {item.caption}
              </span>
            ) : null
      ...
      {showControls && (
        <div className="melt-reel-controls">
          <button type="button" className="melt-reel-btn" aria-label="Previous slide" onClick={handlePrev}>
          ...
          <button type="button" className="melt-reel-btn" aria-label="Next slide" onClick={handleNext}>
            <svg viewBox="0 0 24 24" width="18" height="18" aria-hidden="true">
              <path
                d="M9 5l7 7-7 7"
                fill="none"
      ...
      {showIndicators && (
        <div className="melt-reel-indicators" role="tablist" aria-label="Slides">
          {items.map((item, i) => (
            <button
              key={i}
              type="button"
              ...
              className={`melt-reel-dot ${i === index ? 'is-active' : ''}`}
              onClick={() => {
