...
import './PerspectiveRail.css';

export type PerspectiveRailItem = string | { image: string; alt?: string };
type TiltDirection = 'left' | 'right';

export interface PerspectiveRailProps {
  items?: PerspectiveRailItem[];
  cardWidth?: number;
  cardHeight?: number;
  radius?: number;
  tint?: string;
...
const DEFAULT_ITEMS: PerspectiveRailItem[] = [
...
const normalizeItem = (it: PerspectiveRailItem) => (typeof it === 'string' ? { image: it, alt: '' } : it);

const PerspectiveRail = ({
  items = DEFAULT_ITEMS,
  cardWidth = 300,
  cardHeight = 380,
  radius = 18,
  ...
  return (
    <div
      ref={rootRef}
      className={`perspective-rail ${className}`.trim()}
      ...
      onKeyDown={onKeyDown}
    >
      <div className="perspective-rail__stage" ref={stageRef}>
        {data.map((item, i) => (
          <div
            key={i}
            className="perspective-rail__card"
            ref={el => {
          ...
          >
            <img className="perspective-rail__img" src={item.image} alt={item.alt || ''} draggable={false} />
            <span
              className="perspective-rail__tint"
              ref={el => {
      ...
      {showControls && count > 1 && (
        <>
          <button
            type="button"
            className="perspective-rail__arrow perspective-rail__arrow--prev"
          ...
          <button
            type="button"
            className="perspective-rail__arrow perspective-rail__arrow--next"
      ...
      {showIndicators && count > 1 && (
        <div className="perspective-rail__dots" role="tablist" aria-label="Slides">
          {data.map((_, i) => (
            <button
              key={i}
              type="button"
              ...
              className={`perspective-rail__dot${active === i ? ' is-active' : ''}`}
...
export default PerspectiveRail;
