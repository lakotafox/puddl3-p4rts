...
export interface AfterimageTypeProps {
  text?: string;
  echoes?: number;
  lag?: number;
  offset?: number;
...
const AfterimageType: React.FC<AfterimageTypeProps> = ({
  text = 'Motion Echo',
  echoes = 12,
  lag = 0.24,
  offset = 36,
  ...
  return (
    <span ref={rootRef} className={`afterimage-type ${className}`.trim()} style={rootStyle}>
      {copyIndexes
        .slice(1)
        .reverse()
        .map(index => (
          <span
            aria-hidden="true"
            className="afterimage-type__echo"
      ...
      <span
        className="afterimage-type__echo afterimage-type__echo--front"
        data-echo-index="0"
        ref={element => {
