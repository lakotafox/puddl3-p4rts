...
import './ExtrudedType.css';

export interface ExtrudedTypeProps {
  text?: string;
  layers?: number;
  depth?: number;
  faceColor?: string;
...
const ExtrudedType = ({
  ...
  const rootStyle = {
    ...style,
    '--extruded-type-perspective': `${safePerspective}px`,
    '--extruded-type-font-size': fontSize,
    '--extruded-type-font-weight': fontWeight,
    '--extruded-type-face-color': faceColor,
    '--extruded-type-depth-color': depthColor,
    '--extruded-type-shadow': shadow
      ? `0 22px 34px color-mix(in srgb, ${depthColor} 36%, transparent), 0 4px 8px rgba(0, 0, 0, 0.28)`
      : 'none'
  } as CSSProperties;

  return (
    <span ref={rootRef} className={`extruded-type ${className}`.trim()} style={rootStyle}>
      <span ref={stageRef} className="extruded-type__stage">
        {depthLayers.map(layer => (
          <span
            aria-hidden="true"
            className="extruded-type__layer"
            key={layer.index}
        ...
        <span className="extruded-type__face">{text}</span>
      </span>
    </span>
  );
};

export default ExtrudedType;
