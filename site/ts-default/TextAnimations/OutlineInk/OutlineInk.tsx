...
import './OutlineInk.css';

if (typeof window !== 'undefined') {
  gsap.registerPlugin(ScrollTrigger);
}

export type OutlineInkTrigger = 'mount' | 'hover' | 'scroll' | 'loop';
export type OutlineInkFillMode = 'wipe' | 'fade' | 'none';

export interface OutlineInkProps {
  text?: string;
  strokeColor?: string;
  fillColor?: string;
  strokeWidth?: number;
  ...
  stagger?: number;
  ease?: string;
  trigger?: OutlineInkTrigger;
  fillMode?: OutlineInkFillMode;
  fontSize?: number;
...
const OutlineInk = ({
  ...
  const [box, setBox] = useState<OutlineInkBox | null>(null);

  const rawId = useId();
  const wipeId = `outline-ink-wipe-${rawId.replace(/[^a-zA-Z0-9_-]/g, '')}`;
  ...
  return (
    <span
      ref={rootRef}
      className={`outline-ink ${trigger === 'hover' ? 'outline-ink--hover' : ''} ${className}`.trim()}
      style={{ ...style, ['--outline-ink-height']: `${Math.round(fontSize * 1.3)}px` } as CSSProperties}
      role="img"
      aria-label={String(text ?? '')}
    >
      <svg className="outline-ink__svg" viewBox={viewBox} preserveAspectRatio="xMidYMid meet" aria-hidden="true">
        {fillMode === 'wipe' && box && (
          <defs>
