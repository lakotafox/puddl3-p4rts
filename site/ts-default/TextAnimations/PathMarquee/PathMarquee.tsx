...
import './PathMarquee.css';

export type PathMarqueeShape = 'wave' | 'circle' | 'infinity' | 'arch' | 'line';
export type PathMarqueeDirection = 'forward' | 'reverse';

export interface PathMarqueeProps {
  text?: string;
  shape?: PathMarqueeShape;
  path?: string;
  speed?: number;
  direction?: PathMarqueeDirection;
  separator?: string;
  curviness?: number;
  fontSize?: number;
...
const buildPath = (shape: PathMarqueeShape, curviness: number, ribbonWidth: number): string => {
...
const PathMarquee = ({
  ...
  const rawId = useId();
  const pathId = `path-marquee-${rawId.replace(/:/g, '')}`;
  ...
  return (
    <div ref={rootRef} className={`path-marquee ${className}`.trim()} style={style}>
      <svg
        className="path-marquee-svg"
      ...
      >
        ...
        <text ref={measureRef} className="path-marquee-measure" style={textStyle} aria-hidden="true">
          {unit}
        </text>

        <text className="path-marquee-text" style={textStyle} fill={color} dominantBaseline="central" aria-hidden="true">
