...
interface WarpDriveProps {
  effectOptions?: Partial<WarpDriveOptions>;
}

const defaultOptions: WarpDriveOptions = {
  onSpeedUp: () => {},
  onSlowDown: () => {},
...
class LightsSticks {
  ...
  constructor(webgl: App, options: WarpDriveOptions) {
    this.webgl = webgl;
    this.options = options;
  }
...
class App {
  ...
  constructor(container: HTMLElement, options: WarpDriveOptions) {
    this.options = options;
...
const DEFAULT_EFFECT_OPTIONS: Partial<WarpDriveOptions> = {};

const WarpDrive: FC<WarpDriveProps> = ({ effectOptions = DEFAULT_EFFECT_OPTIONS }) => {
  const warpDrive = useRef<HTMLDivElement>(null);
  ...
  useEffect(() => {
    if (appRef.current) {
      ...
      appRef.current = null;
      const container = warpDrive.current;
      if (container) {
    ...
    const options: WarpDriveOptions = {
      ...defaultOptions,
      ...effectOptions,
  ...
  return <div id="lights" className="w-full h-full" ref={warpDrive}></div>;
};
