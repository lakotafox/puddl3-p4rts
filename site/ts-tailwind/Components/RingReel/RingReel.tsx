...
function deriveFontFamilyFromUrl(url: string): string {
  ...
  return base.replace(/[^a-zA-Z0-9-_ ]/g, '').trim() || 'RingReelFont';
}
...
async function resolveFont(font: string, fontUrl?: string): Promise<string> {
  ...
  try {
    ...
    console.error('RingReel: unable to load font from', fontUrl, error);
    return font;
  }
}
...
export default function RingReel({
  items,
  bend = 3,
  textColor = '#ffffff',
  borderRadius = 0.05,
