...
export interface DepartureBoardProps extends HTMLAttributes<HTMLDivElement> {
  words?: string[];
  text?: string;
  flipDuration?: number;
  stagger?: number;
...
const styles = `
.departure-board{font-family:'SFMono-Regular','Roboto Mono','Cascadia Code','Liberation Mono',Menlo,monospace;font-size:var(--split-flap-font-size,52px);font-weight:760;line-height:1;letter-spacing:.035em;font-variant-numeric:tabular-nums}
.departure-board__tile{position:relative;width:.78em;height:1.08em;overflow:hidden;border-radius:var(--split-flap-radius,8px);background:radial-gradient(circle at 50% 0%,rgba(255,255,255,.16),transparent 44%),linear-gradient(180deg,color-mix(in srgb,var(--split-flap-tile-color,#111827) 82%,white),var(--split-flap-tile-color,#111827));box-shadow:0 .035em .08em rgba(255,255,255,.08) inset,0 -.05em .1em rgba(0,0,0,.38) inset,0 .16em .38em rgba(0,0,0,.28);perspective:520px;transform-style:preserve-3d;isolation:isolate}
.departure-board__tile:before{content:'';position:absolute;z-index:8;top:calc(50% - .5px);left:0;width:100%;height:1px;background:linear-gradient(90deg,transparent,rgba(255,255,255,.18) 18%,rgba(0,0,0,.64) 50%,rgba(255,255,255,.14) 82%,transparent);box-shadow:0 -1px 0 rgba(255,255,255,.08),0 1px 0 rgba(0,0,0,.5);pointer-events:none}
.departure-board__tile:after{content:'';position:absolute;inset:0;z-index:9;border:1px solid rgba(255,255,255,.08);border-radius:inherit;box-shadow:0 0 0 1px rgba(0,0,0,.2) inset;pointer-events:none}
.departure-board__half,.departure-board__flap{position:absolute;left:0;width:100%;height:50%;overflow:hidden;background:linear-gradient(180deg,rgba(255,255,255,.07),transparent 34%),var(--split-flap-tile-color,#111827);backface-visibility:hidden}
.departure-board__half--top,.departure-board__flap--front{top:0}
.departure-board__half--bottom,.departure-board__flap--back{bottom:0;background:linear-gradient(0deg,rgba(255,255,255,.06),transparent 38%),color-mix(in srgb,var(--split-flap-tile-color,#111827) 92%,black)}
.departure-board__char{position:absolute;left:0;width:100%;height:200%;display:flex;align-items:center;justify-content:center;color:var(--departure-board-color,#f8fafc);text-shadow:0 .025em 0 rgba(255,255,255,.16),0 .09em .16em rgba(0,0,0,.42)}
.departure-board__half--top .departure-board__char,.departure-board__flap--front .departure-board__char{top:0}
.departure-board__half--bottom .departure-board__char,.departure-board__flap--back .departure-board__char{bottom:0}
.departure-board__flap{z-index:6;will-change:transform,filter;transform-style:preserve-3d}
.departure-board__flap--front{transform-origin:center bottom;animation:split-flap-front var(--split-flap-flip-duration,.12s) cubic-bezier(.23,1,.32,1) both}
.departure-board__flap--back{transform-origin:center top;transform:rotateX(90deg);animation:split-flap-back var(--split-flap-flip-duration,.12s) cubic-bezier(.23,1,.32,1) both}
...
@media (prefers-reduced-motion:reduce){.departure-board__flap{animation:none!important}}
`;

const toCssUnit = (value: number | string) => (typeof value === 'number' ? `${value}px` : value);

const resolveCharset = (charset: DepartureBoardProps['charset']) => {
...
const DepartureBoard = ({
  ...
  const componentStyle: CSSProperties & Record<string, string | number | undefined> = {
    ...
    '--departure-board-color': textColor,
  ...
  return (
    <>
      <style>{styles}</style>
      <div
        className={`departure-board inline-flex items-center whitespace-pre select-none ${className}`.trim()}
      ...
      >
        {tiles.map((tile, index) => (
          <span className="departure-board__tile" aria-hidden="true" key={`${index}-${tiles.length}`}>
            <span className="departure-board__half departure-board__half--top">
              <span className="departure-board__char">{tile.current === ' ' ? '\u00A0' : tile.current}</span>
            </span>
            <span className="departure-board__half departure-board__half--bottom">
              <span className="departure-board__char">{tile.flipping ? tile.next : tile.current}</span>
            </span>

            {tile.flipping && (
              <>
                <span
                  className="departure-board__flap departure-board__flap--front"
                  key={`front-${index}-${tile.tick}`}
                >
                  <span className="departure-board__char">{tile.current === ' ' ? '\u00A0' : tile.current}</span>
                </span>
                <span className="departure-board__flap departure-board__flap--back" key={`back-${index}-${tile.tick}`}>
                  <span className="departure-board__char">{tile.next === ' ' ? '\u00A0' : tile.next}</span>
                </span>
              </>
            )}
          </span>
