...
export interface OrigamiTypeProps {
  text?: string;
  splitBy?: SplitBy;
  hinge?: Hinge;
  duration?: number;
...
const renderWhitespace = (value: string, key: string): ReactNode[] =>
  value.split(/(\n)/).map((part, index) => {
    ...
    return (
      <span className="origami-type-whitespace" key={`${key}-space-${index}`}>
        {part.replace(/ /g, '\u00A0')}
      </span>
    );
  });

const FOLD_TEXT_STYLES = `.origami-type {
  display: inline-block;
  color: var(--origami-type-color, currentColor);
  font-size: var(--origami-type-font-size, inherit);
  font-weight: var(--origami-type-font-weight, inherit);
  line-height: 0.95;
...
.origami-type-whitespace {
  display: inline;
}

.origami-type-segment {
  display: inline-block;
  line-height: inherit;
...
.origami-type-segment[data-fold-split='line'] {
  display: block;
}
...
const OrigamiType = ({
  ...
  const segments = useMemo(() => {
    ...
    if (splitBy === 'line') {
      return text.split('\n').map((line, index) => (
        <span className="origami-type-line" key={`line-${index}`}>
  ...
  useEffect(() => {
    ...
    const pieces = Array.from(root.querySelectorAll<HTMLElement>('.origami-type-piece'));
  ...
  const rootStyle: CSSProperties = {
    '--origami-type-font-size': typeof fontSize === 'number' ? `${fontSize}px` : fontSize,
    '--origami-type-font-weight': fontWeight,
    '--origami-type-color': color,
    ...style
  } as CSSProperties;
  ...
  return (
    <>
      ...
      <span ref={rootRef} className={`origami-type ${className}`.trim()} style={rootStyle}>
        <span className="origami-type-sr-only">{text}</span>
        <span className="origami-type-visual" aria-hidden="true">
          {segments}
        </span>
      </span>
    </>
