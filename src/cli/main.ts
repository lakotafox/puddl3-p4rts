...
const HELP = `${bold("p4rts")} — foxbits: offline vault of UI components, blocks and templates
...
async function main() {
  switch (cmd) {
    ...
    case "doctor": {
      ...
      checks.push(["vault exists", await exists(join(HOME, "vault")), "vault/ is missing"]);
