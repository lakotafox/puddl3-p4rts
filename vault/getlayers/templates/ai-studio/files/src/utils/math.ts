...
export const interpolate = (
  ...
  for (const key in start) {
    ...
    if (startVal.unit !== null || endVal.unit !== null) {
      ...
      const value = lerp(startVal.number, endVal.number, progress);
      if (unit?.includes("(")) {
        // CSS transform function, e.g. unit "scale()" or "translateY(%)" →
        // reconstruct as fn(value + innerUnit): "scale(42)" / "translateY(-7%)".
        const fn = unit.slice(0, unit.indexOf("("));
        const innerUnit = unit.slice(unit.indexOf("(") + 1, unit.lastIndexOf(")"));
        result[key] = `${fn}(${value}${innerUnit})`;
      } else {
