...
export const Inview = forwardRef<HTMLElement, SpringProps & { tag?: Tags }>(
  (
    ...
    // Until hydration completes, render the plain `from` object so the server
    // and first client paint serialize identically (see `useHydrated`).
    const springStyle = hydrated ? springs : from;
      ...
      return (
        <AnimatedVarTextTag
          ...
          ref={(node) => {
            ...
            // `inViewRef` is the callback ref returned by useDynamicInView —
            // call it so the IntersectionObserver attaches (do not set .current).
            inViewRef(node as HTMLElement);
          }}
          style={{ ...style }}
          {...props}
        >
