...
export interface FlockPointerProps extends React.HTMLAttributes<HTMLDivElement> {
  color?: string;
  accentColor?: string;
  count?: number;
  size?: number;
...
const FlockPointer = ({
  ...
  useEffect(() => {
    ...
    gl.canvas.className = 'flock-pointer__canvas';
  ...
  return (
    <div ref={containerRef} className={`flock-pointer ${className}`.trim()} style={style} {...rest}>
      {children ? <div className="flock-pointer__content">{children}</div> : null}
    </div>
  );
};

export default FlockPointer;
