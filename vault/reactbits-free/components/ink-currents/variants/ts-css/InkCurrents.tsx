...
import './InkCurrents.css';

export interface InkCurrentsProps {
  mouseForce?: number;
  cursorSize?: number;
  isViscous?: boolean;
  viscous?: number;
...
export default function InkCurrents({
  mouseForce = 20,
  cursorSize = 100,
  isViscous = false,
  viscous = 30,
...
}: InkCurrentsProps): React.ReactElement {
  const mountRef = useRef<HTMLDivElement | null>(null);
  const webglRef = useRef<InkCurrentsWebGL | null>(null);
...
`;
    ...
    class WebGLManager implements InkCurrentsWebGL {
      props: any;
      output!: Output;
  ...
  return <div ref={mountRef} className={`ink-currents-container ${className || ''}`} style={style} />;
}
