...
import './RimlightButton.css';

type ButtonSize = 'sm' | 'md' | 'lg';

export interface RimlightButtonProps {
  children?: ReactNode;
  size?: ButtonSize;
  radius?: number;
  tint?: string;
...
const RimlightButton = ({
  ...
  return (
    <button
      ...
      disabled={disabled}
      onClick={onClick}
      className={`rimlight-button rimlight-button--${size}${className ? ` ${className}` : ''}`}
      style={
        {
          '--sb-radius': `${radius}px`,
          '--sb-tint': tint,
    ...
    >
      <span ref={fxRef} className="rimlight-button__fx" aria-hidden="true" />
      <span className="rimlight-button__label">{children}</span>
    </button>
  );
};

export default RimlightButton;
