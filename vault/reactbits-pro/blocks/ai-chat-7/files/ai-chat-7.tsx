...
import { ArrowDown, ArrowUp, Pin, Plus, Square } from "lucide-react";
...
type Message =
  ...
  | { id: string; role: "assistant"; segments: Segment[] };
...
const SOURCES = [
  { n: 1, name: "q3-board-deck.pdf", meta: "PDF · 24 pages" },
  { n: 2, name: "revenue-model.xlsx", meta: "Spreadsheet · updated Oct 2" },
  { n: 3, name: "churn-analysis.md", meta: "Note · 1,200 words" },
  { n: 4, name: "renewals-export.csv", meta: "Data · 8,400 rows" },
  { n: 5, name: "qbr-notes.md", meta: "Note · 640 words" },
  { n: 6, name: "pricing-change-brief.pdf", meta: "PDF · 6 pages" },
  { n: 7, name: "support-tickets.csv", meta: "Data · 2,100 rows" },
  { n: 8, name: "win-back-playbook.md", meta: "Note · 900 words" },
  { n: 9, name: "exec-summary.docx", meta: "Doc · 3 pages" },
  { n: 10, name: "segment-cohorts.xlsx", meta: "Spreadsheet · 14 tabs" },
];

const src = (n: number): Segment => ({ cite: n });

const STREAM_TARGET =
  "Net revenue retention slipped to 104% in Q3 after two mid-market " +
  "downgrades. A renewal-timed win-back should recover most of the gap.";
...
const INITIAL_MESSAGES: Message[] = [
  {
    id: "u1",
    role: "user",
    text: "Summarize why net revenue retention dropped in Q3.",
  },
  {
    id: "a1",
    role: "assistant",
    segments: [
      {
        text: "Net revenue retention fell to 104% from 111% the prior quarter.",
      },
      src(1),
      {
        text: " The decline is concentrated in the mid-market segment, where two large accounts downgraded after a reorganization.",
      },
      src(2),
      {
        text: " Expansion held up in enterprise, but new upsells slowed after the pricing change.",
      },
      src(3),
    ],
  },
  {
    id: "u2",
    role: "user",
    text: "What would recover it fastest?",
  },
  {
    id: "a2",
    role: "assistant",
    segments: [
      {
        text: "The quickest lever is re-engaging the two downgraded mid-market accounts, which together account for four of the seven points lost.",
      },
      src(2),
      {
        text: " A targeted win-back offer before renewal would address most of the gap, and reversing the upsell slowdown means revisiting the new tiers.",
      },
      src(3),
    ],
  },
  {
    id: "u2b",
    role: "user",
    text: "Which segment held up best?",
  },
  {
    id: "a2b",
    role: "assistant",
    segments: [
      {
        text: "Enterprise held up best: expansion there was roughly flat, so almost all of the drag is coming from mid-market rather than a broad-based decline.",
      },
      src(3),
      {
        text: " Gross retention in enterprise actually improved slightly, which is why the net number didn't fall further.",
      },
      src(4),
    ],
  },
  {
    id: "u3",
    role: "user",
    text: "Draft a one-line summary for the board.",
  },
  {
    id: LIVE_ID,
    role: "assistant",
    segments: [
      {
        text: "Net revenue retention slipped to 104% in Q3 after two mid-market downgrades.",
      },
      src(1),
      {
        text: " A renewal-timed win-back should recover most of the gap.",
      },
      src(2),
    ],
  },
...
function Cite({ n }: { n: number }) {
  return (
    <button
      type="button"
      aria-label={`Source ${n}`}
      className="cursor-pointer ml-0.5 inline-flex h-5 min-w-5 translate-y-[-1px] items-center justify-center rounded-[var(--rb-r-sm,6px)] bg-neutral-100 px-1 align-middle text-[11px] tabular-nums text-neutral-600 transition-[transform,background-color] duration-100 ease-[cubic-bezier(0.23,1,0.32,1)] hover:bg-neutral-200 active:scale-[0.97] focus-visible:outline-none focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[var(--rb-accent,oklch(20.5%_0_0))] dark:bg-neutral-800 dark:text-neutral-400 dark:hover:bg-neutral-700 dark:focus-visible:outline-[var(--rb-accent,oklch(100%_0_0))]"
    >
      {n}
    </button>
  );
...
export default function AiChat7() {
  const reduced = useReducedMotion();
  ...
  return (
    <div className="relative flex h-full min-h-[640px] w-full min-w-0 flex-col overflow-hidden bg-white lg:flex-row dark:bg-neutral-950">
      <aside className="hidden shrink-0 flex-col bg-white p-4 lg:flex lg:w-80 dark:bg-neutral-950">
        <h2 className="mb-2 flex items-center gap-2 px-1 text-[13px] font-medium text-neutral-900 dark:text-neutral-100">
          <Pin className="h-3.5 w-3.5 shrink-0 text-neutral-500" />
          Pinned sources
        </h2>
            ...
            >
              {SOURCES.map((s) => (
                <button
                  key={s.n}
                  type="button"
                  className="cursor-pointer flex w-full shrink-0 items-center gap-2.5 rounded-[var(--rb-r-sm,6px)] px-2 py-2 text-left transition-[transform,background-color] duration-100 ease-[cubic-bezier(0.23,1,0.32,1)] hover:bg-neutral-100 active:scale-[0.99] focus-visible:outline-none focus-visible:outline-2 focus-visible:outline-offset-[-2px] focus-visible:outline-[var(--rb-accent,oklch(20.5%_0_0))] dark:hover:bg-neutral-800 dark:focus-visible:outline-[var(--rb-accent,oklch(100%_0_0))]"
                >
                  <span className="inline-flex h-5 min-w-5 shrink-0 items-center justify-center rounded-[var(--rb-r-sm,6px)] bg-neutral-100 px-1 text-[11px] tabular-nums text-neutral-600 dark:bg-neutral-800 dark:text-neutral-400">
                    {s.n}
                  </span>
