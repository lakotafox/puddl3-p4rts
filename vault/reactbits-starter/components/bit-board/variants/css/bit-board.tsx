...
import "./bit-board.css";

export interface BitBoardProps {
  /** Container width */
  width?: string | number;
  /** Container height */
...
const BitBoard: React.FC<BitBoardProps> = ({
  width = "100%",
  height = "100%",
  className,
  children,
  ...
  return (
    <div
      ref={containerRef}
      className={`bit-board-container ${className || ""}`}
      style={{ width, height, backgroundColor }}
      onPointerMove={handlePointerMove}
    >
      <Canvas
        className="bit-board-canvas"
        orthographic
        camera={{
          position: [0, 0, 1],
          zoom: 1,
      ...
      {children && <div className="bit-board-content">{children}</div>}
    </div>
  );
};
