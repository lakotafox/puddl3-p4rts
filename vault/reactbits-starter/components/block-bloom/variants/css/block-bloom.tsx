...
export interface BlockBloomProps {
  color?: string;
  accent?: string;
  background?: string;
  relief?: number;
...
export const BlockBloom = ({
  ...
  return (
    <div
      ref={shell}
      className={["block-bloom", className].filter(Boolean).join(" ")}
      style={style}
    >
      <div className="block-bloom-canvas">
        {awake && box.w > 0 ? (
          <Canvas
            orthographic
      ...
      {children ? <div className="block-bloom-content">{children}</div> : null}
    </div>
  );
};

export default BlockBloom;
