...
import "./block-wipe.css";

export type BlockWipeEasing = "linear" | "easeIn" | "easeOut" | "easeInOut";

export type BlockWipeDirection = "up" | "down" | "left" | "right";
...
export interface BlockWipeProps {
  ...
  easing?: BlockWipeEasing;
  /** Axis along which the reveal sweeps */
  direction?: BlockWipeDirection;
...
const AXIS_INDEX: Record<BlockWipeDirection, number> = {
  up: 0,
  down: 1,
  left: 2,
  right: 3,
...
function easeProgress(t: number, easing: BlockWipeEasing): number {
...
const BlockWipe = React.forwardRef<BlockWipeRef, BlockWipeProps>(
  (
    {
      imageSrc,
      width = "100%",
    ...
    return (
      <div
        ref={containerRef}
        className={["block-wipe-root", className].filter(Boolean).join(" ")}
        style={{
          width,
          height,
          borderRadius: brVal,
      ...
      >
        ...
        {children && <div className="block-wipe-content">{children}</div>}
      </div>
    );
  },
);

BlockWipe.displayName = "BlockWipe";

export default BlockWipe;
