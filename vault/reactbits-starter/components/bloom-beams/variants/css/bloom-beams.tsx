...
import "./bloom-beams.css";

export interface BloomBeamsProps {
  /** Container width */
  width?: string | number;
  /** Container height */
...
const BloomBeams: React.FC<BloomBeamsProps> = ({
  width = "100%",
  height = "100%",
  className = "",
  children,
  ...
  return (
    <div
      ref={containerRef}
      className={`bloom-beams-container ${className}`}
      style={{ width, height }}
      onPointerMove={handlePointerMove}
    >
      <Canvas
        className="bloom-beams-canvas"
      ...
      {children && <div className="bloom-beams-content">{children}</div>}
    </div>
  );
};

BloomBeams.displayName = "BloomBeams";

export default BloomBeams;
