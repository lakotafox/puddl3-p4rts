...
import "./chroma-columns.css";

export interface ChromaColumnsProps {
  /** Container width */
  width?: string | number;
  /** Container height */
...
const ChromaColumns: React.FC<ChromaColumnsProps> = ({
  width = "100%",
  height = "100%",
  className,
  children,
  ...
  return (
    <div
      ref={containerRef}
      className={`chroma-columns-container ${className || ""}`}
      style={{ width, height }}
      onPointerMove={handlePointerMove}
    >
      <Canvas
        className="chroma-columns-canvas"
      ...
      {children && <div className="chroma-columns-content">{children}</div>}
    </div>
  );
};

ChromaColumns.displayName = "ChromaColumns";
