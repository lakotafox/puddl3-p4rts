...
import "./coarse-focus.css";

export interface CoarseFocusProps {
...
const CoarseFocus: React.FC<CoarseFocusProps> = ({
  image,
  pixelSize = 20,
  cursorRadius = 200,
  falloff = 0.5,
  ...
  const webglRef = useRef<CoarseFocusWebGL | null>(null);
  ...
  useEffect(() => {
    ...
    class WebGLManager implements CoarseFocusWebGL {
  ...
  return (
    <div
      ref={mountRef}
      className={`coarse-focus-container ${className}`}
      style={style}
    />
  );
};

export default CoarseFocus;
