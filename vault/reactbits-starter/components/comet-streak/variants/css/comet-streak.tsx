...
export interface CometStreakProps {
  /** Container width */
  width?: string | number;
  /** Container height */
...
interface CometStreakSceneProps {
  speed: number;
  scale: number;
  warpStrength: number;
  warpCurvature: number;
...
const CometStreakScene: React.FC<CometStreakSceneProps> = ({
  speed,
  scale,
  warpStrength,
  warpCurvature,
...
const CometStreak: React.FC<CometStreakProps> = ({
  width = "100%",
  height = "100%",
  className,
  children,
  ...
  return (
    <div
      ref={containerRef}
      className={`comet-streak-container ${className || ""}`}
      style={{ width, height, backgroundColor }}
      onPointerMove={handlePointerMove}
    >
      <Canvas
        className="comet-streak-canvas"
        orthographic
        camera={{
          position: [0, 0, 1],
          zoom: 1,
      ...
      {children && <div className="comet-streak-content">{children}</div>}
    </div>
  );
};

CometStreak.displayName = "CometStreak";

export default CometStreak;
