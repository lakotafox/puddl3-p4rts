...
export interface ConsoleMosaicProps {
  /** Container width */
  width?: string | number;
  /** Container height */
...
const ConsoleMosaic = ({
  ...
  return (
    <div
      ref={rootRef}
      className={["console-mosaic", className].filter(Boolean).join(" ")}
    ...
    >
      <Canvas
        className="console-mosaic-canvas"
        dpr={range}
      ...
      {children ? (
        <div className="console-mosaic-content">{children}</div>
      ) : null}
    </div>
  );
};

export default ConsoleMosaic;
