...
export interface CurvedTickerProps {
...
const Sequence = ({ items, separator, gap, sway, spin }: SequenceProps) => (
  <div className="curved-ticker-sequence" style={{ gap }}>
    {items.map((phrase, index) => (
      <span
        key={`${phrase}-${index}`}
        className="curved-ticker-item"
        style={{ gap }}
      >
        <span
          className="curved-ticker-mark"
          style={
            sway > 0
              ? {
...
const CurvedTicker = ({
  ...
  return (
    <div
      ref={shell}
      className={["curved-ticker", className].filter(Boolean).join(" ")}
    ...
    >
      ...
      >
        {facets.map((facet) => (
          ...
          >
            <div className="curved-ticker-column" style={{ gap: rowGap }}>
