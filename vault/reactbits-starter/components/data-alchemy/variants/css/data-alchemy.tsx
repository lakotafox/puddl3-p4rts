...
import "./data-alchemy.css";

export interface DataAlchemyDocument {
  /** Stable id used as React key. */
  id: string;
}

export interface DataAlchemyResult {
  /** Stable id used as React key. */
  id: string;
...
export interface DataAlchemyClassNames {
  root?: string;
  document?: string;
  axis?: string;
  center?: string;
...
export interface DataAlchemyProps {
  ...
  classNames?: DataAlchemyClassNames;
...
const DEFAULT_DOCUMENTS: DataAlchemyDocument[] = [
  { id: "doc-0" },
  { id: "doc-1" },
  { id: "doc-2" },
  { id: "doc-3" },
...
const DEFAULT_RESULTS: DataAlchemyResult[] = [
...
const DocumentBody = memo(function DocumentBody({
  ...
  if (variant === "image") {
    ...
    return (
      <div className="data-alchemy-doc-body">
        <div className="data-alchemy-doc-header" />
        <div
          className="data-alchemy-doc-image-wrap"
          style={{ height: blockHeight }}
        >
          <HalftoneBlock width={innerWidth} height={blockHeight} />
        </div>
        <div className="data-alchemy-doc-paragraph" style={{ marginTop: 8 }}>
  ...
  return (
    <div className="data-alchemy-doc-body">
      <div className="data-alchemy-doc-header" />
      <div className="data-alchemy-doc-paragraph">
      ...
      <div className="data-alchemy-doc-input-row">
        <div className="data-alchemy-doc-input" style={{ width: "36%" }} />
        <div className="data-alchemy-doc-input" style={{ width: "18%" }} />
      </div>
      <div className="data-alchemy-doc-paragraph">
      ...
      <div className="data-alchemy-doc-input-row">
        <div className="data-alchemy-doc-input" style={{ width: "28%" }} />
        <div className="data-alchemy-doc-bar" style={{ width: "24%" }} />
      </div>
      <div className="data-alchemy-doc-paragraph">
...
const ResultBody = () => (
  <div className="data-alchemy-result-body">
    {Array.from({ length: 3 }).map((_, i) => (
      <div key={i} className="data-alchemy-result-row">
        <div className="data-alchemy-result-cell" />
        <div className="data-alchemy-result-cell--wide" />
      </div>
    ))}
  </div>
);
...
const SlidingDoc = memo(function SlidingDoc({
  ...
  const docStyle: CSSProperties = {
    ...
    animationName: "data-alchemy-doc-slide",
  ...
  const docClassName = ["data-alchemy-doc", className]
    .filter(Boolean)
    .join(" ");

  return (
    <div className={docClassName} style={docStyle}>
      <DocumentBody
        seed={index + 1}
        variant={variant}
        width={documentWidth}
...
const DataAlchemy = ({
  ...
  const rootClassName = ["data-alchemy-root", classNames?.root, className]
    .filter(Boolean)
    .join(" ");

  return (
    <div ref={stageRef} className={rootClassName} style={rootStyle}>
      {stageWidth > 0 && (
        <div
          key={restartKey}
          className="data-alchemy-doc-clip"
          style={{
            width: centerX,
      ...
      <div
        className={["data-alchemy-axis", classNames?.axis]
          .filter(Boolean)
          .join(" ")}
        style={{
          width: axisHaloWidth,
      ...
      />
      <div
        className="data-alchemy-axis-line"
        style={{ height: axisHeight, background: axisColor }}
        aria-hidden
      />

      <motion.div
        key={`center-${burstId}`}
        className={["data-alchemy-center", classNames?.center]
          .filter(Boolean)
          .join(" ")}
        style={{
          width: centerSize,
      ...
      {stageWidth > 0 && showBursts && (
        <div
          ...
          className="data-alchemy-particle-layer"
          aria-hidden
        >
          {particleSpecs.map((p) => {
            ...
            return (
              <motion.div
                key={p.id}
                className={["data-alchemy-particle", classNames?.particle]
                  .filter(Boolean)
                  .join(" ")}
                style={{
                  width: p.size,
              ...
              >
                <div
                  className="data-alchemy-particle-shape"
                  style={{
                    background: p.color,
      ...
      {stageWidth > 0 && showBursts && (
        <div
          key={`results-${burstId}`}
          className="data-alchemy-result-layer-spawn"
          aria-hidden
        >
          {results.map((res, i) => {
            ...
            return (
              <motion.div
                key={res.id}
                className={["data-alchemy-result", classNames?.result]
                  .filter(Boolean)
                  .join(" ")}
                style={{
              ...
              >
                <div
                  className="data-alchemy-chip"
                  style={{ background: res.color }}
                />
                <div className={classNames?.resultBody}>
                  <ResultBody />
                </div>
              </motion.div>
            );
      ...
      {stageWidth > 0 && paused && (
        <div className="data-alchemy-result-layer-spawn" aria-hidden>
          {results.map((res, i) => {
