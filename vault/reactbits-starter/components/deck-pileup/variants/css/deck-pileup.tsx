...
export type DeckPileupVariant =
  "stack" | "deck" | "fade" | "flip" | "zoom" | "reveal";

export interface DeckPileupItem {
...
const DEFAULT_ITEMS: DeckPileupItem[] = [
  {
    eyebrow: "01. Survey",
...
export const DeckPileup = ({
  ...
  return (
    <section
      ...
      className={["deck-pileup", className].filter(Boolean).join(" ")}
      style={{ height: `${runway}vh` }}
    >
      <div
        className="deck-pileup-pin"
        style={{ perspective: `${Math.max(200, perspective)}px` }}
      >
        <div
          className="deck-pileup-frame"
          style={{
        ...
        {(showProgress || showCounter) && count > 1 && (
          <div className="deck-pileup-hud">
            {showProgress && (
              <span className="deck-pileup-rail">
                <span
                  ref={railRef}
                  className="deck-pileup-fill"
                  style={{ transform: "scaleX(0)" }}
                />
              </span>
            )}
            {showCounter && (
              <span className="deck-pileup-count">
    ...
    {item.image && (
      <>
        <img
          ...
          decoding="async"
          draggable={false}
          className="deck-pileup-image"
        />
        <span className="deck-pileup-scrim" />
      </>
    )}

    <span className={`deck-pileup-index${item.image ? " is-over" : ""}`}>
      {String(index + 1).padStart(2, "0")}/{String(total).padStart(2, "0")}
    </span>

    <div className="deck-pileup-copy">
      {item.eyebrow && (
        <span
          className={`deck-pileup-eyebrow${item.image ? " is-over" : ""}`}
      ...
      {item.title && (
        <h3 className={`deck-pileup-title${item.image ? " is-over" : ""}`}>
          {item.title}
        </h3>
      )}
      {item.body && (
        <p className={`deck-pileup-body${item.image ? " is-over" : ""}`}>
          {item.body}
        </p>
      )}
    </div>
