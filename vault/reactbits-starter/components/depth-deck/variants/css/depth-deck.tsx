...
import "./depth-deck.css";

export interface DepthDeckProps {
...
const DepthDeck = ({
  ...
  return (
    <div
      ref={wrapperRef}
      className={`depth-deck-container ${className}`}
      style={{
      ...
      >
        {visibleImages.map((imageUrl, idx) => (
          <button
            ...
            type="button"
            className={`parallax-card ${focusedCardIndex === idx ? "parallax-card-focused" : ""}`}
            style={{
