...
export interface EndlessWallProps {
  /** Container width */
  width?: string | number;
  /** Container height */
...
function loadTex(url: string, onReady?: () => void): THREE.Texture | null {
  ...
  texLoader.load(
    ...
    undefined,
    () => {
      console.warn("EndlessWall: failed to load", url);
    },
  );
  return null;
}
...
const EndlessWall: React.FC<EndlessWallProps> = ({
  width = "100%",
  height = "100%",
  className,
  images,
  ...
  return (
    <div
      className={`endless-wall-container ${className || ""}`}
      style={{ width, height }}
    >
      <Canvas
        className="endless-wall-canvas"
        camera={{
          position: [0, 0, 50],
          fov: 60,
          near: 1,
...
EndlessWall.displayName = "EndlessWall";

export default EndlessWall;
