...
const FluidColumns: React.FC<FluidColumnsProps> = ({
  width = "100%",
  height = "100%",
  speed = 1,
  color = "#a855f7",
  ...
  const containerClassName = className
    ? `fluid-columns-container ${className}`
  ...
  return (
    ...
    >
      <Canvas
        className="fluid-columns-canvas"
        orthographic
        camera={{
          left: -1,
          right: 1,
      ...
      {children && <div className="fluid-columns-content">{children}</div>}
    </div>
  );
};
