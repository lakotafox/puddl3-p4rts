...
const GlowSweep: React.FC<GlowSweepProps> = ({
  revealDelay = 0,
  revealDuration = 2000,
  verticalOffset = 0.7,
  ...
  return (
    <div ref={containerRef} className={`glow-sweep-container ${className}`}>
      <canvas ref={canvasRef} className="glow-sweep-canvas" />
      {children && <div className="glow-sweep-content">{children}</div>}
    </div>
  );
};

GlowSweep.displayName = "GlowSweep";
