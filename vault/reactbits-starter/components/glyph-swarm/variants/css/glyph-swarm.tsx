...
export interface GlyphSwarmProps {
  text?: string;
  className?: string;
  colors?: string[];
  particleSize?: number;
...
const GlyphSwarm: React.FC<GlyphSwarmProps> = ({
  text = "brilliant.",
  className = "",
  ...
  return (
    <div ref={containerRef} className={`glyph-swarm-container ${className}`}>
      <canvas ref={canvasRef} className="glyph-swarm-canvas" />
    </div>
  );
};

GlyphSwarm.displayName = "GlyphSwarm";

export default GlyphSwarm;
