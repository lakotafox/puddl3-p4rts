...
export interface HoloFlipProps {
...
const DEFAULT_ITEMS: HoloFlipItem[] = [
  {
  ...
  <div className="holo-flip-rail">
    {items.map((item, index) => (
      <button
        ...
        className="holo-flip-tick"
      >
        <span
          aria-hidden
          className="holo-flip-fill"
...
const HoloFlip: React.FC<HoloFlipProps> = ({
  items = DEFAULT_ITEMS,
  initialIndex = 2,
  cardWidth = 260,
  aspectRatio = "3 / 4",
  ...
  return (
    <div
      ...
      onKeyDown={onKeyDown}
      className={["holo-flip", className].filter(Boolean).join(" ")}
    >
      <div
        ref={stage}
        ...
        className={[
          "holo-flip-stage",
          enableDrag && count > 1 ? "is-draggable" : "",
        ]
          .filter(Boolean)
          .join(" ")}
      >
      ...
      {(showControls || showDots) && count > 1 && (
        <div className="holo-flip-controls">
          {showControls && (
