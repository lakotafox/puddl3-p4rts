...
const InkWashScene: React.FC<InkWashSceneProps> = ({
  speed,
  scale,
  octaves,
  persistence,
...
const InkWash: React.FC<InkWashProps> = ({
  ...
  return (
    <div
      ref={containerRef}
      className={`ink-wash-container ${className}`}
      style={{ width, height }}
    ...
    >
      ...
      {children && <div className="ink-wash-content">{children}</div>}
    </div>
  );
};

InkWash.displayName = "InkWash";
