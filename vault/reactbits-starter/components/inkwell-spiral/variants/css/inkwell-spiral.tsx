...
import "./inkwell-spiral.css";

export interface InkwellSpiralProps {
  /** Container width */
  width?: string | number;
  /** Container height */
...
const InkwellSpiral: React.FC<InkwellSpiralProps> = ({
  width = "100%",
  height = "100%",
  className,
  children,
  ...
  return (
    <div
      ref={containerRef}
      className={`inkwell-spiral-container ${className || ""}`}
      style={{ width, height }}
      onPointerMove={handlePointerMove}
    >
      <Canvas
        className="inkwell-spiral-canvas"
      ...
      {children && <div className="inkwell-spiral-content">{children}</div>}
    </div>
  );
};

InkwellSpiral.displayName = "InkwellSpiral";

export default InkwellSpiral;
