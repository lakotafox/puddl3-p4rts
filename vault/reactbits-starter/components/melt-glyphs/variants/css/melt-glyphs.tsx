...
import "./melt-glyphs.css";

export interface MeltGlyphsProps {
  /** Container width */
  width?: string | number;
  /** Container height */
...
const MeltGlyphs: React.FC<MeltGlyphsProps> = ({
  width = "100%",
  height = "100%",
  className,
  children,
  ...
  return (
    <div
      ref={wrapRef}
      className={`melt-glyphs-container ${className || ""}`}
      style={{ width, height, backgroundColor }}
    >
      <pre
        ref={preRef}
        className="melt-glyphs-pre"
        style={{
          fontFamily,
      ...
      />
      {children && <div className="melt-glyphs-content">{children}</div>}
    </div>
  );
};

MeltGlyphs.displayName = "MeltGlyphs";

export default MeltGlyphs;
