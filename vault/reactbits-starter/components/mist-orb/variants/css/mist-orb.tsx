...
type PipelineProps = Required<
  Omit<MistOrbProps, "width" | "height" | "className" | "children" | "dpr">
>;
...
const MistOrb: React.FC<MistOrbProps> = ({
  width = "100%",
  height = "100%",
  className = "",
  children,
  ...
  return (
    <div
      className={`mist-orb-container ${className}`.trim()}
      style={{ width, height }}
    >
      <Canvas
        className="mist-orb-canvas"
        dpr={[1, dpr]}
        gl={{
          antialias: false,
          alpha: true,
      ...
      {children && <div className="mist-orb-content">{children}</div>}
    </div>
  );
};
