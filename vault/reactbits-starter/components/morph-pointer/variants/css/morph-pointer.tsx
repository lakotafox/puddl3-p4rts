...
import "./morph-pointer.css";

export interface MorphPointerProps {
...
const MorphPointer: React.FC<MorphPointerProps> = ({
  circleSize = 40,
  dotSize = 6,
  ...
  return (
    <div className={`morph-pointer-container ${className}`} style={{ zIndex }}>
      <motion.div
        className={`morph-pointer-circle ${circleClassName}`}
        style={{
          width: circleWidth,
          height: circleHeight,
      ...
      <motion.div
        className={`morph-pointer-dot ${dotClassName}`}
        animate={
          activeTarget !== null
            ? {
                opacity: 0,
        ...
        return (
          <AnimatePresence key={`${selector}-${index}`}>
            {isHovered && (
              ...
              >
                <div
                  className={`morph-pointer-image ${imageClassName}`}
                  style={{
