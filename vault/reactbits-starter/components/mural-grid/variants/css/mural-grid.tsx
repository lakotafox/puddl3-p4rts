...
import "./mural-grid.css";

export interface MuralGridProps {
  ...
  color3?: string;
  /** Pixel/mural-grid tile size (1-50) */
  pixelSize?: number;
  /** MuralGrid border intensity (0-1) */
...
const MuralGrid: React.FC<MuralGridProps> = ({
  width = "100%",
  height = "100%",
  speed = 1.0,
  variant = "shader",
  ...
  return (
    <div
      className={`mural-grid-container ${className || ""}`}
      style={{
        width: widthStyle,
        height: heightStyle,
      }}
    >
      <div ref={containerRef} className="mural-grid-canvas" />
      {children && <div className="mural-grid-content">{children}</div>}
    </div>
  );
};

MuralGrid.displayName = "MuralGrid";
