...
import "./nebula-card.css";

export interface NebulaCardProps {
...
const NebulaCard: React.FC<NebulaCardProps> = ({
  ...
  return (
    <div
      className={`nebula-card ${className}`}
      style={{
        width: `${width}px`,
        height: `${height}px`,
    ...
    >
      <div
        ref={containerRef}
        className="nebula-card-canvas"
        style={{
          filter: blur > 0 ? `blur(${blur}px)` : undefined,
          opacity: opacity,
        }}
      />
      {children && <div className="nebula-card-content">{children}</div>}
    </div>
  );
};

NebulaCard.displayName = "NebulaCard";
