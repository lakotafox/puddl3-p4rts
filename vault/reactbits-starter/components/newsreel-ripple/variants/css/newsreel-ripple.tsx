...
import "./newsreel-ripple.css";

export interface NewsreelRippleProps {
  /** Container width */
  width?: string | number;
  /** Container height */
...
const NewsreelRipple: React.FC<NewsreelRippleProps> = ({
  width = "100%",
  height = "100%",
  className,
  children,
  ...
  return (
    <div
      ref={containerRef}
      className={`newsreel-ripple-container ${className || ""}`}
      style={{ width, height, backgroundColor }}
      onPointerMove={handlePointerMove}
    >
      <Canvas
        className="newsreel-ripple-canvas"
        orthographic
        camera={{
          position: [0, 0, 1],
          zoom: 1,
      ...
      {children && <div className="newsreel-ripple-content">{children}</div>}
    </div>
  );
};

NewsreelRipple.displayName = "NewsreelRipple";

export default NewsreelRipple;
