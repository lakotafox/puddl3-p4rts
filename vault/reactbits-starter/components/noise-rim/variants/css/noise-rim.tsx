...
import "./noise-rim.css";

export interface NoiseRimProps {
  /** Container width */
  width?: string | number;
  /** Container height */
...
const NoiseRim: React.FC<NoiseRimProps> = ({
  width = "100%",
  height = "100%",
  className,
  children,
  ...
  return (
    <div
      className={`noise-rim-container ${className || ""}`}
      style={{ width, height }}
    >
      <Canvas
        className="noise-rim-canvas"
        orthographic
        camera={{
          position: [0, 0, 1],
          zoom: 1,
      ...
      {children && <div className="noise-rim-content">{children}</div>}
    </div>
  );
};

NoiseRim.displayName = "NoiseRim";

export default NoiseRim;
