...
import "./oblique-columns.css";

export interface ObliqueColumnsProps {
...
const Strip = ({
  ...
  return (
    <div className="oblique-columns-strip" style={{ paddingTop: `${lead}%` }}>
      <motion.div className="oblique-columns-track" style={{ y, rowGap: gap }}>
        {tiled.map((src, index) => (
          <div
            key={`${src}-${index}`}
            className="oblique-columns-tile"
            style={{
              aspectRatio: aspect,
              borderRadius: radius,
...
const ObliqueColumns: React.FC<ObliqueColumnsProps> = ({
  ...
  return (
    <div
      className={`oblique-columns ${className}`.trim()}
      style={{
        width,
        height,
        perspective,
    ...
    >
      <motion.div
        ref={planeRef}
        className="oblique-columns-plane"
