...
export interface PanBoardProps {
...
const PanBoard: React.FC<PanBoardProps> = ({
  items,
  columns = 10,
  enableDrag = true,
  enableWheel = true,
  ...
  useGSAP(
    () => {
      ...
      requestAnimationFrame(() => {
        ...
        timeline.set(containerRef.current, { scale: 0.5 });
  ...
  return (
    <div
      ref={containerRef}
      className={`pan-board-container ${containerMode === "fixed" ? "fixed-mode" : "absolute-mode"} ${className}`}
    >
      <div
        ref={gridRef}
        className={`pan-board ${isLoaded && enableDrag && !isDragging ? "draggable" : ""} ${isDragging ? "dragging" : ""} ${gridClassName}`}
        style={{ columnGap: gapSize }}
      >
        {columnedItems.map((column, colIndex) => (
          <div
            key={colIndex}
            className="pan-board-column"
            style={{
              rowGap: gapSize,
          ...
          >
            {column.map((item, itemIndex) => {
              ...
              return (
                <div
                  key={item.id}
                  className={`product ${itemClassName}`}
