...
import "./pointer-ripple.css";

export type PointerRippleShape = "circle" | "triangle" | "square";

export type PointerRippleColor = string | { stops: [string, string] };

export interface PointerRippleProps {
  /** Container width */
  width?: string | number;
  /** Container height */
...
export interface PointerRippleHandle {
  /** Trigger a wave burst at an explicit (clientX, clientY). Defaults to centre. */
  burst: (x?: number, y?: number) => void;
}

const DEFAULT_COLORS: PointerRippleColor[] = [
  "#22c55e",
  "#06b6d4",
  "#f97316",
  "#ef4444",
...
const DEFAULT_SHAPES: PointerRippleShape[] = ["circle", "triangle", "square"];
...
const PointerRipple = React.forwardRef<PointerRippleHandle, PointerRippleProps>(
  (
    {
      width = "100%",
      height = "100%",
    ...
    const rootClass = `pointer-ripple-root${className ? ` ${className}` : ""}`;
    ...
    return (
      ...
      >
        <canvas ref={canvasRef} className="pointer-ripple-canvas" />
        {children && <div className="pointer-ripple-content">{children}</div>}
      </div>
    );
  },
);

PointerRipple.displayName = "PointerRipple";
