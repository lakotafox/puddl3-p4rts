...
type SceneProps = Required<
  Omit<PrismGridProps, "width" | "height" | "className" | "children" | "dpr">
>;
...
const PrismGrid: React.FC<PrismGridProps> = ({
  width = "100%",
  height = "100%",
  className,
  children,
  ...
  return (
    <div
      className={["prism-grid-root", className].filter(Boolean).join(" ")}
      style={{ width, height }}
    >
      <Canvas
        className="prism-grid-canvas"
        dpr={[1, dpr]}
        gl={{
          antialias: false,
          alpha: true,
      ...
      {children && <div className="prism-grid-content">{children}</div>}
    </div>
  );
};

PrismGrid.displayName = "PrismGrid";
