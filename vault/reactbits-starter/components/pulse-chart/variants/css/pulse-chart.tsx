...
const PulseChart = ({
  ...
  return (
    <div
      ref={containerRef}
      className={`pulse-chart pulse-chart--neutral ${className || ""}`}
      style={{ width: widthStyle, height: `${height}px` }}
    >
      <svg
        ref={svgRef}
        viewBox="0 0 800 400"
        className="pulse-chart__svg"
              ...
              return (
                ...
                >
                  <motion.div
                    ...
                    className="pulse-chart__tooltip-wrapper"
                    style={{ pointerEvents: "none" }}
                  >
                    <div className="pulse-chart__tooltip-inner">
                      <div className="pulse-chart__tooltip-content pulse-chart__tooltip-content--neutral">
                        {calculatePercentageDifference && hoveredIndex > 0 ? (
                          (() => {
                            const diff = getPercentageDifference(hoveredIndex);
                            if (!diff) {
                              return (
                                <div className="pulse-chart__tooltip-value">
                                  {points[hoveredIndex].value.toFixed(2)}
                                </div>
                              );
                            }
                            return (
                              <div className="pulse-chart__tooltip-percentage">
                                {diff.isIncrease ? (
                                  <TrendingUp className="pulse-chart__tooltip-icon pulse-chart__tooltip-icon--up" />
                                ) : (
                                  <TrendingDown className="pulse-chart__tooltip-icon pulse-chart__tooltip-icon--down" />
                                )}
                                <span
                                  className={
                                    diff.isIncrease
                                      ? "pulse-chart__tooltip-value--up"
                                      : "pulse-chart__tooltip-value--down"
                                  }
                                >
                                  {diff.isIncrease ? "+" : "-"}
                        ...
                        {data[hoveredIndex].label && (
                          <div className="pulse-chart__tooltip-label">
                            {data[hoveredIndex].label}
                          </div>
                        )}
                      </div>
                      <div
                        className="pulse-chart__tooltip-arrow"
                        style={{
                          bottom: "-4px",
...
PulseChart.displayName = "PulseChart";
