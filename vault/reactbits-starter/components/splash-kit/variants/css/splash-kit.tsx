...
import "./splash-kit.css";

export interface SplashKitProps {
  /** Whether the splash-kit is active */
  loading: boolean;

  /** Visual variant of the splash-kit */
  variant?: "stairs" | "percentage" | "circle" | "slide" | "curtain";

  /** Position type of the splash-kit container */
...
const SplashKit: React.FC<SplashKitProps> = ({
  loading,
  variant = "stairs",
  position = "absolute",
  duration = 2500,
  ...
  const renderLoadingText = () => {
    ...
    return (
      <div className="splash-kit-loading-text" style={{ zIndex: zIndex + 1 }}>
        <div className="splash-kit-loading-text-wrapper">
          {words.map((word, index) => (
            <motion.span
              ...
              className={`splash-kit-loading-text-word ${textClassName}`}
            >
              {word}
            </motion.span>
          ))}
  ...
  const renderStairsVariant = () => {
    ...
    return (
      <div
        className={`splash-kit-stairs splash-kit-${position}`}
        style={{
          zIndex,
        }}
        role="status"
      ...
      >
        {stairs.map((_, index) => (
          ...
          >
            {!bgColor && (
              <div
                className="splash-kit-stair splash-kit-stair-colored"
  ...
  const renderPercentageVariant = () => {
    ...
    const getPositionClasses = () => {
      if (percentagePosition === "bottom-left") {
        return "splash-kit-percentage-bottom-left";
      } else if (percentagePosition === "top-left") {
        return "splash-kit-percentage-top-left";
      }
      return "";
    };
    ...
    return (
      <motion.div
        ...
        className={`splash-kit-percentage splash-kit-${position} ${getPositionClasses()} ${bgColor ? "" : "splash-kit-percentage-bg"}`}
        style={{
          zIndex,
      ...
      >
        <div
          className={`splash-kit-percentage-number ${percentageTextClassName}`}
        >
          {displayProgress}
          {showPercentageSign && (
            <span className="splash-kit-percentage-symbol">%</span>
          )}
        </div>

        {showProgressBar && (
          <div
            className={`splash-kit-percentage-bar-container ${progressBarPosition === "top" ? "splash-kit-percentage-bar-top" : ""}`}
          >
            <motion.div
              initial={{ width: "0%" }}
              animate={{ width: `${progress}%` }}
              transition={{ duration: 0.05, ease: "linear" }}
              className="splash-kit-percentage-bar"
            />
          </div>
        )}
      </motion.div>
  ...
  const renderCircleVariant = () => {
    ...
    return (
      <div
        className={`splash-kit-circle splash-kit-${position}`}
        style={{ zIndex }}
        role="status"
        aria-label={ariaLabel}
        aria-live={ariaLive}
      >
        <motion.div
          ...
          className={`splash-kit-circle-shape ${bgColor ? "" : "splash-kit-circle-colored"}`}
          style={{
            width: "300vmax",
            height: "300vmax",
            aspectRatio: "1",
    ...
    return (
      ...
      >
        ...
        >
          {!bgColor && (
            <div
              className="splash-kit-stair-colored"
              style={{ width: "100%", height: "100%" }}
            />
          )}
        </motion.div>
  ...
  return (
    <div className={`splash-kit-wrapper ${className}`}>
      <AnimatePresence onExitComplete={onLoadingComplete}>
        {showPreloader && (
          <div key="splash-kit">
            {customContent ? (
              <div
                className={`splash-kit-custom splash-kit-${position}`}
                style={{ zIndex }}
                role="status"
                aria-label={ariaLabel}
                aria-live={ariaLive}
      ...
      <div
        className={`splash-kit-content ${showPreloader ? "splash-kit-content-hidden" : ""}`}
      >
        {children}
      </div>
    </div>
...
SplashKit.displayName = "SplashKit";

export default SplashKit;
