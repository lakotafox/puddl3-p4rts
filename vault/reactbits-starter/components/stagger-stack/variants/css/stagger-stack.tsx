...
import "./stagger-stack.css";

export interface StaggerStackItem {
...
export interface StaggerStackProps {
  /** Array of items to display */
  items: StaggerStackItem[];
  ...
  onDismiss?: (item: StaggerStackItem) => void;
  /** Callback when an item is clicked */
  onItemClick?: (item: StaggerStackItem) => void;
  /** Gap between items in pixels */
  itemGap?: number;
  /** Additional CSS classes */
  className?: string;
  /** Custom item render function */
  renderItem?: (item: StaggerStackItem) => React.ReactNode;
...
const Ripple: React.FC<{ x: number; y: number; onComplete: () => void }> = ({
  ...
  return (
    <motion.span
      className="stagger-stack-ripple"
...
const StaggerStackItemComponent: React.FC<{
  item: StaggerStackItem;
  itemRenderer: (item: StaggerStackItem) => React.ReactNode;
  hoverEffect: string;
  clickEffect: string;
  swipeToDismiss: boolean;
  onDismiss?: (item: StaggerStackItem) => void;
  ...
  return (
    <motion.div
      ref={itemRef}
      className={`stagger-stack-item-card ${onItemClick ? "stagger-stack-item-clickable" : ""}`}
    ...
    >
      {swipeToDismiss && (
        <motion.div
          className="stagger-stack-swipe-indicator"
      ...
      <motion.div
        className="stagger-stack-item-content"
...
const StaggerStack: React.FC<StaggerStackProps> = ({
  items: initialItems,
  duration = 0.4,
  ...
  const [items, setItems] = useState<StaggerStackItem[]>(() => initialItems);
  ...
  useEffect(() => {
    ...
    intervalRef.current = setInterval(() => {
      ...
      setItems((prevItems) => {
        ...
        const newItem: StaggerStackItem = {
  ...
  const defaultRenderItem = (item: StaggerStackItem) => (
    <div className="stagger-stack-item-text">{item.content}</div>
  );
  ...
  const paddingClass =
    startFrom === "center"
      ? "stagger-stack-items-center"
      : startFrom === "bottom"
        ? "stagger-stack-items-bottom"
        : "";

  return (
    <div
      className={`stagger-stack-container ${className || ""}`}
      style={{ height }}
    >
      {fadeEdges && (
        <>
          <div
            className="stagger-stack-fade-top"
            style={{
              height: fadeEdgeSize,
          ...
          />
          <div
            className="stagger-stack-fade-bottom"
            style={{
              height: fadeEdgeSize,
      ...
      <div
        className="stagger-stack-scroll"
        onMouseEnter={() => pauseOnHover && setIsPaused(true)}
        onMouseLeave={() => pauseOnHover && setIsPaused(false)}
      >
        <ul
          className={`stagger-stack-items ${paddingClass}`}
