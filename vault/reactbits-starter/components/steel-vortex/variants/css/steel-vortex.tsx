...
const SteelVortex: React.FC<SteelVortexProps> = ({
  ...
  return (
    <div
      ref={containerRef}
      className={`steel-vortex-container ${className || ""}`}
      style={{ width, height, backgroundColor }}
      onPointerMove={handlePointerMove}
    >
      <Canvas
        className="steel-vortex-canvas"
        orthographic
        camera={{
          position: [0, 0, 1],
          zoom: 1,
      ...
      {children && <div className="steel-vortex-content">{children}</div>}
    </div>
  );
};
