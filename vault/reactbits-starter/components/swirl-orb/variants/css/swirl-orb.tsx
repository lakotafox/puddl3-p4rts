...
import "./swirl-orb.css";

export interface SwirlOrbProps {
  /** Container width */
  width?: string | number;
  /** Container height */
...
const SwirlOrb: React.FC<SwirlOrbProps> = ({
  width = "100%",
  height = "100%",
  className = "",
  children,
  ...
  return (
    <div
      className={`swirl-orb-container ${className}`}
      style={{
        width,
        height,
      }}
    >
      ...
      {children && <div className="swirl-orb-overlay">{children}</div>}
    </div>
  );
};

SwirlOrb.displayName = "SwirlOrb";
