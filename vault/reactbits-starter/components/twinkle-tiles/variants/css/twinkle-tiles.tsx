...
import "./twinkle-tiles.css";

export type TwinkleTilesDirection = "right" | "left" | "top" | "bottom";

export interface TwinkleTilesProps {
  /** Container width */
  width?: string | number;
  /** Container height */
   ...
   */
  direction?: TwinkleTilesDirection;
...
function dirToVec(dir: TwinkleTilesDirection): [number, number] {
  switch (dir) {
    case "left":
      return [-1, 0];
    case "top":
...
const TwinkleTiles: React.FC<TwinkleTilesProps> = ({
  width = "100%",
  height = "100%",
  className,
  children,
  ...
  return (
    <div
      className={["twinkle-tiles-root", className].filter(Boolean).join(" ")}
      style={{ width, height }}
    >
      <Canvas
...
TwinkleTiles.displayName = "TwinkleTiles";

export default TwinkleTiles;
