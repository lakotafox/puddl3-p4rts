...
const TypeBlock: React.FC<TypeBlockProps> = ({
  width = "100%",
  height = "100%",
  className,
  children,
  ...
  useEffect(() => {
    ...
    const loop = (now: number) => {
      ...
      for (let fi = 0; fi < 6; fi++) {
        let center: Vec3 = {
          x: FACE_CENTERS_UNIT[fi].x * side,
          y: FACE_CENTERS_UNIT[fi].y * side,
          z: FACE_CENTERS_UNIT[fi].z * side,
        };
        let up = { ...FACE_UP[fi] };
        ...
        for (let gi = 0; gi < grid; gi++) {
          for (let gj = 0; gj < grid; gj++) {
            ...
            const proj = project(pt, cfg.perspective);
  ...
  return (
    <div
      ref={wrapRef}
      className={`type-block-container ${className || ""}`}
      style={{ width, height, backgroundColor }}
    >
      <canvas ref={canvasRef} className="type-block-canvas" />
      {children && <div className="type-block-content">{children}</div>}
    </div>
  );
};
