...
import "./type-route.css";

export interface TypeRouteProps {
  text: string;
  path: string;
  className?: string;
  duration?: number;
...
const TypeRoute: React.FC<TypeRouteProps> = ({
  ...
  const id = useId();
  const pathId = `type-route-curve-${id.replace(/:/g, "")}`;
  ...
  return (
    <div ref={containerRef} className={`type-route-container ${className}`}>
      <svg
        viewBox={viewBox}
        xmlns="http://www.w3.org/2000/svg"
        className="type-route-svg"
...
TypeRoute.displayName = "TypeRoute";
