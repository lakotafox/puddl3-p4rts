...
import "./vanishing-lattice.css";

export interface VanishingLatticeProps {
...
const VanishingLattice: React.FC<VanishingLatticeProps> = ({
  width = "100%",
  height = 400,
  speed = 0.5,
  color = "#FF9FFC",
  ...
  return (
    <div
      className={`vanishing-lattice-container ${className}`}
      style={{
        width: widthStyle,
        height: heightStyle,
      }}
    >
      <div ref={containerRef} className="vanishing-lattice-canvas" />
      {bottomFade && (
        <div
          className="vanishing-lattice-bottom-fade"
          style={{
            background: `linear-gradient(to top, ${bottomFade}, transparent)`,
          }}
        />
      )}
      {children && <div className="vanishing-lattice-content">{children}</div>}
    </div>
  );
};

VanishingLattice.displayName = "VanishingLattice";

export default VanishingLattice;
