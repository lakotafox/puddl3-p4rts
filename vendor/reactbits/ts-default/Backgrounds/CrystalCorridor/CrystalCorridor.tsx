...
import './CrystalCorridor.css';

export type CrystalCorridorDetail = 'low' | 'medium' | 'high';

export interface CrystalCorridorProps {
  color1?: string;
  color2?: string;
  color3?: string;
  detail?: CrystalCorridorDetail;
  speed?: number;
  waveDepth?: number;
  zoom?: number;
  density?: number;
...
const DETAIL_STEPS: Record<CrystalCorridorDetail, number> = { low: 20, medium: 32, high: 48 };
const stepsFor = (detail: CrystalCorridorDetail): number => DETAIL_STEPS[detail] || DETAIL_STEPS.medium;
...
type CrystalCorridorCtx = {
  renderer: InstanceType<typeof Renderer>;
  program: InstanceType<typeof Program>;
  mesh: InstanceType<typeof Mesh>;
};
const ctxMap = new WeakMap<HTMLDivElement, CrystalCorridorCtx>();

const CrystalCorridor: React.FC<CrystalCorridorProps> = ({
  color1 = '#5227FF',
  color2 = '#A855F7',
  color3 = '#FFFFFF',
  detail = 'medium',
  ...
  return <div ref={containerRef} className={`crystal-corridor-container ${className}`.trim()} />;
};

export default CrystalCorridor;
