...
import './ForgeSwirl.css';

export type ForgeSwirlColorMode = 'molten' | 'ember' | 'frost';

export interface ForgeSwirlProps {
  color1?: string;
  color2?: string;
  color3?: string;
  speed?: number;
...
const colorModeToFloat = (mode: ForgeSwirlColorMode): number => (mode === 'ember' ? 1 : mode === 'frost' ? 2 : 0);
...
const ctxMap = new WeakMap<HTMLDivElement, ForgeSwirlCtx>();

const ForgeSwirl: React.FC<ForgeSwirlProps> = ({
  color1 = '#5227FF',
  color2 = '#FF9FFC',
  color3 = '#FFFFFF',
  speed = 0.35,
  ...
  return <div ref={containerRef} className={`forge-swirl-container ${className}`.trim()} />;
};

export default ForgeSwirl;
