...
import './ChoiceDial.css';

type Side = 'left' | 'right';

export interface ChoiceDialProps {
  items?: string[];
...
const ChoiceDial = ({
  ...
  return (
    <div
      ...
      className={`choice-dial${side === 'right' ? ' choice-dial--right' : ''}${isDragging ? ' choice-dial--dragging' : ''}${className ? ` ${className}` : ''}`}
      style={
        {
    ...
    >
      {items.map((label, index) => (
        <div
          ...
          className={`choice-dial__item${selectedIndex === index ? ' choice-dial__item--selected' : ''}`}
