...
import './TileStream.css';

export interface TileStreamItem {
  image: string;
  title?: string;
  href?: string;
}

export interface TileStreamProps {
  items?: TileStreamItem[];
  columns?: number;
  tileWidth?: number;
  tileHeight?: number;
  gap?: number;
...
const DEFAULT_ITEMS: TileStreamItem[] = Array.from({ length: 15 }, (_, i) => {
...
const TileStream = ({
  ...
  const columnItems = useMemo<TileStreamItem[][]>(() => {
    const cols: TileStreamItem[][] = Array.from({ length: columns }, () => []);
  ...
  const renderTile = (item: TileStreamItem, id: string, colIndex: number) => {
    const inner = (
      <span className="tile-stream__inner">
        <img src={item.image} alt={item.title ?? ''} loading="lazy" decoding="async" draggable={false} />
        <span className="tile-stream__overlay" aria-hidden="true" />
      </span>
    );
    const commonProps = {
      className: `tile-stream__tile${activeId === id ? ' is-active' : ''}`,
      'data-tile-id': id,
      'data-col': colIndex,
  ...
  const rootClass = ['tile-stream', reduced ? 'tile-stream--reduced' : '', className].filter(Boolean).join(' ');
  ...
  return (
    ...
    >
      <div ref={planeRef} className="tile-stream__plane">
        {columnItems.map((col, c) => {
          const meta = columnMeta[c];
          const copies = Array.from({ length: meta.copies });
          return (
            <div className="tile-stream__col" key={`col-${c}`}>
              <div
                className="tile-stream__track"
                ref={el => {
