...
export interface DustTypeProps {
  text?: string;
  particleSize?: number;
  density?: number;
  color?: string;
...
const DustType = ({
  ...
  return (
    <div ref={containerRef} className={`dust-type ${className}`} style={style} aria-label={text}>
      <canvas ref={canvasRef} className="dust-type__canvas" aria-hidden="true" />
      <span className="dust-type__sr">{text}</span>
    </div>
  );
};
