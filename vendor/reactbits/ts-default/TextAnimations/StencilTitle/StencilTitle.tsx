...
export interface StencilTitleProps {
  text?: string;
  tag?: ElementType;
...
const StencilTitle: React.FC<StencilTitleProps> = ({
  ...
  return (
    <TagAny
      ref={rootRef}
      className={`stencil-title ${className}`.trim()}
      style={{
        textAlign: align,
        fontWeight: weight,
      ...
      {...rest}
    >
      <span ref={measureRef} className="stencil-title__measure">
        {words.map((word, i) => (
          <span
            key={`${word}-${i}`}
          ...
          >
            ...
            <i
              ...
              className="stencil-title__baseline"
            />
          </span>
        ))}
      </span>

      <svg className="stencil-title__defs" aria-hidden="true" focusable="false">
        <defs>
      ...
      <span ref={revealRef} className="stencil-title__reveal">
        <span className="stencil-title__clip" style={{ clipPath: `url(#${clipId})` }}>
          <span ref={mediaRef} className="stencil-title__media">
            {mediaType === 'video' ? (
              <video className="stencil-title__source" src={src} poster={poster} autoPlay muted loop playsInline />
            ) : (
              <img className="stencil-title__source" src={src} alt="" draggable={false} />
            )}
          </span>
        </span>
      </span>
