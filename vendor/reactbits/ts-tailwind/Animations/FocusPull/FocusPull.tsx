...
type FocusPullProps = PropsWithChildren<{
...
const FocusPull: React.FC<FocusPullProps> = props => {
  ...
  return (
    <div
      ref={containerRef}
      className={`focus-pull relative isolate ${config.target === 'page' ? 'focus-pull-page' : 'focus-pull-parent'} ${config.className}`}
...
const injectStyles = () => {
  ...
  const id = 'focus-pull-styles';
  if (document.getElementById(id)) return;
  const el = document.createElement('style');
  el.id = id;
  el.textContent = `.focus-pull{pointer-events:none;transition:opacity .3s ease-out}.focus-pull-inner{pointer-events:none}`;
